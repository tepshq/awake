...
**AI を回している間、Mac の蓋を閉じても止めない。**

Claude Code やローカル LLM を長時間ぶん回しているとき、「蓋を閉じても処理を続けたい」——
AWAKE はその状態（= **AWAKE モード**）を、メニューバーから ON / OFF できる SwiftBar プラグインです。

ただし付けっぱなしは発熱・電池切れのもと。だから **今 ON かどうかが一目で分かり**、
**長時間つけっぱなしを「必ずオフ」で止める**ので、安心して回せます。

### こんなときに

- 🤖 **Claude Code や AI エージェントを長時間走らせている**間、蓋を閉じても止めたくない
- 🧠 ローカル LLM・学習・重いバッチを回しっぱなしにしたい

### 特徴

- 🟢 **見える化** — メニューバーに 👀（通常）/ 👁️（AWAKE）で状態を常時表示。切り忘れに気づける
- 🔒 **安全設計** — sudoers 改変も常駐ヘルパーも無し。切り替えるときだけ macOS 標準のパスワードダイアログ
- ⏰ **必ずオフ** — 設定時間を超えると「オフにするまで消えない」ダイアログで付けっぱを防止
- 🚀 **配布がラク** — チームは 1 行コマンドだけ（SwiftBar 導入〜配置まで自動）
...
👀 通常モードにする        ← クリックで OFF
──────────────
オフ強制まで：6時間 ▸      ← ホバーで 1 / 3 / 6 / 12 時間を選択
```
...
AWAKE モードの付けっぱなしを防ぐため、**設定時間（既定 6 時間）を超えると、オフにするまでダイアログが出続けます**。

```
⚠️ AWAKE
AWAKE モードが6時間以上続いています。
安全のため、一度オフにしてください。
            ［オフにする］
```
...
- 時間はパネルの **「オフ強制まで：N時間」** から **1 / 3 / 6 / 12 時間** に変更できます（無効化はできません）。
...
- **蓋を閉じて外部ディスプレイ無しのまま長時間 AWAKE モードにすると発熱しやすい**です。AI を回しっぱなしだと特に熱くなるので、カバンの中などでの放置に注意してください。
- AWAKE モードはあくまで一時的な用途に。**用が済んだら通常モードに戻す**運用を推奨します。
...
- **なぜ「アイコンを 1 クリックで即トグル」ではないの？**
  SwiftBar はメニューバーアイコンをクリックすると必ずパネルが開く仕様で、「タイトルのクリック＝即アクション」はできません。そのためパネル先頭の切り替えボタンを押す **実質 2 クリック**になります。これは「現在の状態をメニューバーに表示する」ための割り切りです。

- **自動でオフにはできないの？**
  OFF も root が必要で、AWAKE は「毎回パスワード」方針のため、**無人で勝手に OFF にすることはできません**。そのため「長時間で必ずオフを促すダイアログ」という形にしています。

- **メニューバーの表示が実際の状態とズレる**
  プラグインは 30 秒ごと（ファイル名の `30s`）＋メニューを開いたときに自動更新します。手動で `pmset` を叩いた直後などは、メニューを開けば最新化されます。
